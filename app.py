...
def build_state(data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "personal_workspace": build_personal_workspace(real_user) if logged_in() and not is_admin() and real_user.get("self_admin") else {},
        "can_personal_admin": bool(real_user.get("self_admin", False)) if logged_in() and not is_admin() else False,
...
async function logout(){try{const data=await api("/api/logout","POST",{});state={...state,...data.state};render();notify(data.message||"Loggedout")}catch(err){notify(err.message,true)}}
