...
async functiondeleteSuggestion(idx){if(!confirm("Delete this suggestion?"))return;try{await api("/api/admin/suggestion/delete","POST",{index:idx});await refreshState();notify("Suggestion deleted")}catch(err){notify(err.message,true)}}
