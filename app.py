...
def get_private_workspace(data: dict[str, Any], username: str, db_name: str) -> dict[str, Any]:
    data.setdefault("private_workspaces", {})
    data["private_workspaces"].setdefault(username, {})
    data["private_workspaces"][username].setdefault(db_name, {})
    return data["private_workspaces"][username][db_name]


def get_private_file_set(data: dict[str, Any], username: str, db_name: str) -> set[str]:
    return set(collect_db_files(get_private_workspace(data, username, db_name)))


def file_is_private_to_user(data: dict[str, Any], username: str, db_name: str, file_name: str) -> bool:
    return file_name in get_private_file_set(data, username, db_name)


def workspace_visible_database(data: dict[str, Any], db_name: str, username: str) -> dict[str, Any]:
    base = filtered_database_for_user(data, db_name, username)
    user = data.get("users", {}).get(username, {})
    if username == "ADMIN" or user.get("self_admin"):
        private = deepcopy(get_private_workspace(data, username, db_name))
        for category_name, category in private.items():
            if category_name in base:
                merged = base[category_name]
                for file_name in category.get("files", []):
                    if file_name not in merged.setdefault("files", []):
                        merged["files"].append(file_name)
                for sub_name, files in category.get("subdivisions", {}).items():
                    merged.setdefault("subdivisions", {}).setdefault(sub_name, [])
                    for file_name in files:
                        if file_name not in merged["subdivisions"][sub_name]:
                            merged["subdivisions"][sub_name].append(file_name)
            else:
                base[category_name] = category
    return base


def can_manage_current_workspace(data: dict[str, Any]) -> bool:
    if is_admin():
        return True
    user = data.get("users", {}).get(get_real_username(), {})
    return bool(user.get("self_admin"))
...
    visible_db = workspace_visible_database(data, active_db, view_user) if logged_in() else data["databases"].get("TOA", {})
...
        "can_manage_workspace": can_manage_current_workspace(data) if logged_in() else False,
...
  self_admin:false,self_default_db:"",self_allowed_dbs:[]
};
...
async function switchDb(targetDb=null){try{let nextDb=targetDb;if(!nextDb){const allowed=state.allowed_dbs||["TOA"];const idx=allowed.indexOf(state.active_db);nextDb=allowed[(idx+1)%allowed.length]}const data=await api("/api/file/"+encodeURIComponent(fileName));state.selectedFile=fileName;state.currentFileContent=data.file_content;state.currentEditNote=data.note;state.isEditingFile=false;render()}catch(err){notify(err.message,true)}}
...
      <div class="box-head"><h3>SELF-ADMIN SETTINGS</h3></div><div class="muted">These changes only affect your own private workspace.</div>
...
  document.getElementById("app").innerHTML=`<div class="wrap"><div class="topbar"><div><h1 class="title">${esc(state.active_db)}_TERMINAL</h1><p class="sub">SECURE CONNECTION ESTABLISHED // ${isAdmin?"ADMIN OVERRIDE ACTIVE":"STANDARD OP LEVEL"}</p>${isAdmin&&state.view_as?`<p class="sub">VIEWING AS: ${esc(state.view_as)}</p>`:""}${!isAdmin&&state.self_admin?`<p class="sub">SELF-ADMIN ENABLED</p>`:""}</div><div class="row">${state.can_suggest?`<button data-action="submit-suggestion">SUGGESTION</button>`:""}${canSwitchDb?`<button data-action="switch-db">SWITCH DATABASE</button>`:""}${isAdmin?`<button data-action="open-info-terminal">INFO TERMINAL</button>`:""}<button class="danger" data-action="logout">TERMINATE</button></div></div><div class="layout"><div class="card panel"><div class="header-row"><h2>SYSTEM STATUS</h2></div><div class="status"><div>LOGGED IN AS:</div><strong>${esc(state.username||"UNKNOWN")}</strong><div>VIEWING AS:</div><strong>${esc(state.viewing_as||state.username||"UNKNOWN")}</strong><div>ACCESS LEVEL:</div><strong>${esc(isAdmin?"OMEGA-PRIME":"OMEGA")}</strong><div>ENCRYPTION:</div><strong>256-BIT QUANTUM</strong><div>DATABASE SYNC:</div><strong>100%</strong><div>ACTIVE NODE:</div><strong>${esc(state.active_db)} ROOT</strong></div></div><div class="card panel"><div class="header-row"><div><h2>MAIN_DIRECTORY</h2><p class="sub">NAVIGATE THE ARCHIVAL RECORDS BELOW.</p></div>${canManageWorkspace?`<button data-action="add-entry">ADD ENTRY</button>`:""}</div><div class="tree-section">${categoriesHtml||'<div class="muted">NO CATEGORIES AVAILABLE FOR THIS USER.</div>'}</div></div></div>${renderSelfBox()}${renderAdminBox()}${renderSuggestionBox()}<div id="notice" class="notice"></div></div><div class="dialog-backdrop ${dialogOpen?"open":""}" id="dialogBackdrop"><div class="card dialog"><div class="header-row"><h2>📄 ${esc(selected||"")}</h2><button class="ghost" data-action="close-dialog">CLOSE</button></div><div class="split"><div class="box"><div class="box-head"><h3>CORE FILE</h3>${canManageWorkspace?`<div class="row">${state.isEditingFile?`<button data-action="save-file">SAVE</button><button class="danger" data-action="cancel-edit">CANCEL</button>`:`<button data-action="start-edit">EDIT</button>`}</div>`:""}</div>${canManageWorkspace&&state.isEditingFile?`<textarea id="fileContentEditor" style="min-height:340px;">${esc(state.currentFileContent||"")}</textarea>`:`<pre>${esc(state.currentFileContent||"")}</pre>`}</div>${noteVisible?`<div class="box"><div class="box-head"><h3>ADMIN NOTES</h3>${isAdmin?`<button data-action="save-note">SAVE NOTE</button>`:""}</div>${isAdmin?`<textarea id="noteEditor" style="min-height:180px;">${esc(state.currentEditNote||"")}</textarea>`:`<pre>${esc((selected&&state.customNotes[selected])||"")}</pre>`}</div>`:""}</div></div></div>`;
  wireMainEvents();
  if(canManageWorkspace&&state.isEditingFile){const ed=document.getElementById("fileContentEditor");if(ed)ed.addEventListener("input",e=>state.currentFileContent=e.target.value)}
...
  if(action==="logout")return logout(); if(action==="switch-db")return switchDb(); if(action==="open-info-terminal")return switchDb("INFO"); if(action==="add-entry")return addEntry(); if(action==="close-dialog")return closeDialog();
...
    data = load_data()
    if not can_manage_current_workspace(data):
        return jsonify({"ok": False, "error": "MANAGE ACCESS REQUIRED"}), 403
...
    view_user = get_view_username()
    can_access = user_can_access_file(data, view_user, active_db, file_name)
    if not can_access and view_user != "ADMIN":
        can_access = file_is_private_to_user(data, view_user, active_db, file_name)
    if not can_access:
...
    body = request.get_json(silent=True) or {}
    active_db = get_active_db(data)
    if not is_admin() and not file_is_private_to_user(data, get_real_username(), active_db, file_name):
        return jsonify({"ok": False, "error": "SELF-ADMIN CAN ONLY EDIT OWN FILES"}), 403
...
    else:
        workspace = get_private_workspace(data, get_real_username(), active_db)
...
    active_db = get_active_db(data)
    db = data["databases"][active_db] if is_admin() else get_private_workspace(data, get_real_username(), active_db)
...
        return jsonify({"ok": False, "error": "MISSING CATEGORY"}), 400
    active_db = get_active_db(data)
    target = data["databases"][active_db] if is_admin() else get_private_workspace(data, get_real_username(), active_db)
...
        data["users"][username]["admin_only_login"] = DEFAULT_USERS[username].get("admin_only_login", False)
