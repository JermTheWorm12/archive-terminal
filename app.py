...
def sort_named_mapping(mapping: dict[str, Any]) -> dict[str, Any]:
    return {name: mapping[name] for name in sorted(mapping.keys(), key=natural_key)}



def sort_category_contents(category: dict[str, Any]) -> None:
    category["files"] = sorted(category.get("files", []), key=natural_key)
    subdivisions = category.get("subdivisions", {})
    for sub_name, files in list(subdivisions.items()):
        subdivisions[sub_name] = sorted(files, key=natural_key)
    category["subdivisions"] = sort_named_mapping(subdivisions)


def sort_database_structure(db: dict[str, Any]) -> dict[str, Any]:
    for category in db.values():
        sort_category_contents(category)
    return sort_named_mapping(db)


def sort_workspace_structure(workspace: dict[str, Any]) -> dict[str, Any]:
    for category in workspace.values():
        sort_category_contents(category)
    return sort_named_mapping(workspace)
        ...
        for db_name in allowed:
            ...
            if perms is None:
                ...
                if "*" in perms:
                    ...
                    user["file_permissions"][db_name] = sorted({name for name in perms if name in valid}, key=natural_key)
...
let state = {
  ...
  selectedFile:null,currentFileContent:"",currentEditNote:"",isEditingFile:false,  categoryOpen:{},subdivisionOpen:{},editingUser:null,userEditor:null,permissionDb:"TOA",
...
async async function cancelSelfDestruct(){
...
function wireMainEvents(){document.querySelectorAll("[data-action]").forEach(el=>{el.addEventListener("click",async e=>{e.preventDefault();e.stopPropagation();const action=el.dataset.action;
  ...
  if(action==="cancel-self-destruct")return cancelSelfDestruct();  if(action==="backup-export")return triggerBackupDownload(); if(action==="start-self-destruct")return startSelfDestruct(); if(action==="confirm-self-destruct")return confirmSelfDestruct(); if(action==="cancel-self-destruct")return cancelSelfDestruct();
...
def api_admin_self_destruct_cancel():
    ...
    return jsonify({"ok": True, "message": "SELF DESTRUCT CANCELED", "state": build_state(refreshed)})
...
def api_add_category():
    ...
    data["databases"][active_db] = sort_database_structure(data["databases"][active_db])
    save_data(data)
    return jsonify({"ok": True})
...
def api_add_subdivision():
    ...
    sort_category_contents(db[category])
    data["databases"][active_db] = sort_database_structure(db)
    save_data(data)
    return jsonify({"ok": True})
...
def api_personal_add_category():
    ...
    if category not in workspace:
        ...
        user["personal_workspace"] = sort_workspace_structure(workspace)
        save_data(data)
    return jsonify({"ok": True})
...
def api_personal_add_subdivision():
    ...
    sort_category_contents(workspace[category])
    user["personal_workspace"] = sort_workspace_structure(workspace)
    save_data(data)
    return jsonify({"ok": True})
