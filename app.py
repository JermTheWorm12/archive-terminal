...
def sort_named_mapping(mapping: dict[str, Any]) -> dict[str, Any]:
    return {name: mapping[name] for name in sorted(mapping.keys(), key=str.casefold)}


def sort_category_contents(category: dict[str, Any]) -> None:
    category["files"] = sorted(category.get("files", []), key=str.casefold)
    subdivisions = category.get("subdivisions", {})
    for sub_name, files in list(subdivisions.items()):
        subdivisions[sub_name] = sorted(files, key=str.casefold)
...
let state = {
  ...
  self_admin:false,self_default_db:"",self_allowed_dbs:[],personal_workspace:{},can_personal_admin:false,selectedFileScope:"shared",  self_destruct:{active:false,end_time:0,backup_required:true}, selfDestructTimer:null
};
